...
const Linetest = React.forwardRef(
  (
    {
      selectedDate,
      dayType,
    ...
    const chartComp = async (timestamps) => {
      if (predictedData) {
        ...
        setChartData([
          {
            id: `Predicted ${metric === 'wdLoad' ? 'Load' : metric === 'temp' ? 'Temp' : 'Humid'} ${selectedDate.format("YYYY/MM/DD")}`,
